...
declare function dashboard:display-applications () {
    ...
    return dashboard:display-package($package)
};
...
declare function dashboard:display-package ($app as element(app)) {
    ...
    let $url := if ($app/url) then
        $app/url
    else
        $app/@path
    return let $installed := $app/@installed/string()
        let $available := $app/@available/string()
        let $hasNewer := if ($app/@available) then
            packages:is-newer($available, $installed)
        else
            false()
        let $status := if ($app/@status = 'installed') then
            'installed'
        else
            'notInstalled'

        return <existdb-package-descriptor
            abbrev="{ $app/abbrev }"
            available="{ $available }"
            data-name="{ $app/name/string() }"
            installed="{ $installed }"
            short-title="{ $app/title/text() }"
            status="{ $status }"
            tabindex="0"
            type="{ $app/type }"
            url="{ $url }"
        >
            ...
            }
            {
                if ($app/@status = 'installed' and $app/type = 'application') then
                    <existdb-app-icon>
                        <a href="{ $url }" tabindex="-1" target="_blank" title="click to open application">
                            <img class="appIcon" src="{ $icon }" />
                        </a>
                    </existdb-app-icon>
                else
                    <existdb-app-icon><img class="appIcon" src="{ $icon }" /></existdb-app-icon>
            }
            <existdb-app-title>{ $app/title/text() }</existdb-app-title>
            {
                if ($hasDetailsLevel and $app/@available) then
                    if ($hasNewer) then (
                        <existdb-app-update available="{ $available }" installed="{ $installed }">
                            {
                                if ($app/changelog/change[@version = $available]) then
                                    <a class="show-changes" data-version="{ $available }" href="#">Changes</a>
                                else (
                                )
                            }
                        </existdb-app-update>,
