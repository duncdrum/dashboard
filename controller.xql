...
if ($exist:path eq '') then
 ...
 :) else if ($exist:resource eq 'login') then
    ...
    return (
        ...
        try {
            <status xmlns:json="http://www.json.org">
                ...
                {
                    if ($user) then (
                        for $item in sm:get-user-groups($user)
                        return <groups json:array="true">{ $item }</groups>,
                        <dba>{ sm:is-dba($user) }</dba>
                    ) else (
                    )
                }
            </status>
        } catch * { response:set-status-code(401), <status>{ $err:description }</status> }
    )
    ...
    :)
    return if ($user and sm:is-dba($user)) then (
        (:
